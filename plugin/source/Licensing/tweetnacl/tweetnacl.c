...
// Source: https://github.com/dominictarr/tweetnacl (public domain)
// Provides: crypto_hash_sha512, crypto_verify_32, crypto_sign_ed25519_open,
//          ed25519_verify_detached (convenience wrapper)
// Stripped of Salsa20, Poly1305, box, secretbox, key generation (not needed).
...
sv st32(u8 *x,u32 u) { int i; FOR(i,4) { x[i] = u; u >>= 8; } }
sv ts64(u8 *x,u64 u) { int i; for (i = 7;i >= 0;--i) { x[i] = u; u >>= 8; } }
...
static int vn(const u8 *x,const u8 *y,int n) {
  u32 i,d = 0; FOR(i,n) d |= x[i]^y[i]; return (1 & ((d - 1) >> 8)) - 1;
}
int crypto_verify_16_tweet(const u8 *x,const u8 *y) { return vn(x,y,16); }
int crypto_verify_32_tweet(const u8 *x,const u8 *y) { return vn(x,y,32); }
...
int crypto_hash_sha512_tweet(u8 *out,const u8 *m,u64 n) {
  ...
  FOR(i,64) h[i] = iv[i];
  crypto_hashblocks(h,m,n); m += n; n &= 127; m -= n;
...
sv set25519(gf r, const gf a) { int i; FOR(i,16) r[i]=a[i]; }
sv car25519(gf o) {
  int i; i64 c;
  FOR(i,16) { o[i]+=(1LL<<16); c=o[i]>>16;
    o[(i+1)*(i<15)]+=c-1+37*(c-1)*(i==15); o[i]-=c<<16; }
}
sv sel25519(gf p,gf q,int b) {
  i64 t,i,c=~(b-1); FOR(i,16) { t=c&(p[i]^q[i]); p[i]^=t; q[i]^=t; }
}
sv pack25519(u8 *o,const gf n) {
  int i,j,b; gf m,t;
  FOR(i,16) t[i]=n[i]; car25519(t); car25519(t); car25519(t);
  FOR(j,2) {
    m[0]=t[0]-0xffed; for(i=1;i<15;i++) {
...
static int neq25519(const gf a, const gf b) {
  u8 c[32],d[32]; pack25519(c,a); pack25519(d,b); return crypto_verify_32_tweet(c,d);
}
static u8 par25519(const gf a) { u8 d[32]; pack25519(d,a); return d[0]&1; }
sv unpack25519(gf o, const u8 *n) {
  int i; FOR(i,16) o[i]=n[2*i]+((i64)n[2*i+1]<<8); o[15]&=0x7fff;
}
sv A(gf o,const gf a,const gf b) { int i; FOR(i,16) o[i]=a[i]+b[i]; }
sv Z(gf o,const gf a,const gf b) { int i; FOR(i,16) o[i]=a[i]-b[i]; }
sv M(gf o,const gf a,const gf b) {
  i64 i,j,t[31]; FOR(i,31) t[i]=0;
  FOR(i,16) FOR(j,16) t[i+j]+=a[i]*b[j]; FOR(i,15) t[i]+=38*t[i+16];
  FOR(i,16) o[i]=t[i]; car25519(o); car25519(o);
}
sv S(gf o,const gf a) { M(o,a,a); }
sv inv25519(gf o,const gf i) {
  gf c; int a; FOR(a,16) c[a]=i[a];
...
sv cswap(gf p[4],gf q[4],u8 b) { int i; FOR(i,4) sel25519(p[i],q[i],b); }
sv pack(u8 *r,gf p[4]) {
  gf tx, ty, zi; inv25519(zi, p[2]); M(tx, p[0], zi); M(ty, p[1], zi);
  pack25519(r, ty); r[31] ^= par25519(tx) << 7;
}
sv scalarmult(gf p[4],gf q[4],const u8 *s) {
  int i; set25519(p[0],gf0); set25519(p[1],gf1);
  set25519(p[2],gf1); set25519(p[3],gf0);
  for (i = 255;i >= 0;--i) {
    u8 b = (s[i/8]>>(i&7))&1; cswap(p,q,b); add(q,p);
    add(p,p); cswap(p,q,b);
  }
}
sv scalarbase(gf p[4],const u8 *s) {
  gf q[4]; set25519(q[0],X); set25519(q[1],Y);
  set25519(q[2],gf1); M(q[3],X,Y); scalarmult(p,q,s);
}
...
sv modL(u8 *r,i64 x[64]) {
  ...
  FOR(i,32) { x[i+1] += x[i] >> 8; r[i] = x[i] & 255; }
}
sv reduce(u8 *r) {
  i64 x[64],i; FOR(i,64) x[i] = (u64) r[i]; FOR(i,64) r[i] = 0; modL(r,x);
}
...
static int unpackneg(gf r[4],const u8 p[32]) {
  ...
  M(t,den6,num); M(t,t,den); pow2523(t,t);
...
int ed25519_verify_detached(const unsigned char sig[64],
  ...
  unsigned long long n = msglen + 64;
  unsigned char sm[4096], m[4096];
  unsigned long long outlen;
  int i, result;
  if (n > sizeof(sm)) return -1;
  FOR(i,32) sm[i] = sig[i];
  FOR(i,32) sm[32+i] = sig[32+i];
  FOR(i,(int)msglen) sm[64+i] = msg[i];

  result = crypto_sign_ed25519_tweet_open(m, &outlen, sm, n, pk);
